...
export class PiParse {
    ...
    set splitter_main(value) {
        this.#MAIN_SPLITTER = value;
    }
    set splitter_entry(value) {
        this.#ENTRY_SPLITTER = value;
    }
    set splitter_parameter(value) {
    ...
    parse(value) {
        ...
        const parameters = this.#handleParametersAndOptions(parameters_values);
        return { name, key, values, parameters };
    }
    parameters(value, keys) {
        return this.#handleParametersAndOptions(value, keys ?? this.#keys);
    }
    #handleNameKeyValues(name_key_values) {
        const [name_key, ...valuesSplitted] = name_key_values.split(' ');
        const [name, key] = name_key.split('=');
        const values = splitValues(valuesSplitted
            .join(' '), this.#ENTRY_SPLITTER).map(value => convertValue(value));
        return { name, key, values };
    }
    #handleParametersAndOptions(parameters_values, keys) {
        if (parameters_values === undefined) {
            return {};
        }
